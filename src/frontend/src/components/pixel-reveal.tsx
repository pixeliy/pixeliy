...
type PixelRevealProps = {
  ...
  easing?: EaseLike;       // string OK, dikonversi ke cubic-bezier
...
// Map string easings → cubic-bezier (yang kompatibel dgn Motion)
...
const PixelReveal: React.FC<PixelRevealProps> = ({
  ...
  // seed stabil biar jitter konsisten per play
  ...
  // === TOP → BOTTOM sweep (tiap baris maju delay-nya)
  // ada sedikit jitter per tile supaya tidak terlalu "garis lurus"
  const delays = useMemo(() => {
    const arr = new Array<number>(total).fill(0);
    const tileAnimSec = 0.35; // durasi tiap tile fade
    ...
    for (let i = 0; i < total; i++) {
      ...
      // jitter kecil: 0..(20% dari rowDelay), plus offset tipis per kolom biar ada “gelombang”
      ...
      <svg width="100%" height="100%" preserveAspectRatio="none" viewBox={`0 0 ${vw} ${vh}`}>
        <defs>
          <mask id={maskId} x="0" y="0" width={vw} height={vh} maskUnits="userSpaceOnUse">
            {/* start putih = overlay hitam terlihat penuh */}
            ...
            {Array.from({ length: total }, (_, i) => {
              ...
              // tile dalam MASK (hitam akan “membolongi” overlay)
              return (
                <motion.rect
                  key={`m-${i}`}
                  x={x}
        ...
        {/* === Aksen lime flash ===
            Ditaruh DI BAWAH overlay hitam, sehingga hanya terlihat
            ketika lubang mask di area itu sudah terbuka */}
        {Array.from({ length: total }, (_, i) => {
          const c = i % colsEff;
          const r = Math.floor(i / colsEff);
          const x = Math.floor(c * tileW);
        ...
        {/* overlay hitam yang dimask → jadi “pixel reveal” */}
        <motion.rect
          x="0"
          y="0"
          width={vw}
